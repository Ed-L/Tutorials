...
# Background
I used this workflow to analyze 16s amplicon data from the Grinnell CAFO project, but this tutorial should be applicable to any 16s data.

In order to understand this tutorial, you will need a basic understanding of bash, and R. If you are want to learn either or need a review, see the helpful references tab.

You will also need the following installed on an AWS instance or HPC (See references for help):
...
## Running MOTHUR
I thought about writing an original tutorial for using MOTHUR, but it would really be redundant. Pat Schloss and co. made an [amazing tutorial](https://mothur.org/wiki/EDAMAME) and, now that you are set up, you should be able to follow it exactly. Using the defaults should work well for our purposes. For this tutorial, you will only need to follow the tutorial through to the "Analysis" section, where you rename files right before subsampling, but you are welcome to complete the MOTHUR tutorial. Once you have files called "stability.opti_mcc.shared" and "stability.cons.taxonomy" you are good to move on to the next section.
...
* I recommend doing the analysis in one session (might require skill with [tmux](https://github.com/edamame-course/2015-tutorials/blob/master/final/2015-06-22_tmux.md)), but you can do MOTHUR in multiple sessions. Make sure you are calling the right directories (I used [set.dir](https://mothur.org/wiki/Set.dir) a lot).
...
* MOTHUR takes up a huge amount of space. It outputs lots of text files per a sequence, and each sequence is already taking up a large amount of memory. Try restarting on a larger instance with more memory. To analyze about 150 sequences, I had to use a m5.12xlarge and added on even more storage. NOTE: THE LARGER THE INSTANCE THE MORE IT COSTS.
...
So now that you have "stability.opti_mcc.shared", you should boot up Rstudio and are ready for analysis. For the purposes of this tutorial, I will show EVERYTHING - both the r code input (in gray) and raw output (in white) including warning messages. A lot of this is modified from the [Phyloseq tutorial](https://joey711.github.io/phyloseq/tutorials-index.html).

First, load the libraries. Some of these may be redundant, but thats a problem to solve at a later date.
```{r, echo=TRUE}
library(ape)
library(grid)
library(plyr)
...
Lets import the data. You should also make and import a meta data .csv file. The meta file will contain details about each sample.
```{r, echo=TRUE}
...
Well there isn't a right answer. Currently, I've heard rarefying at 10,000 sequences is ideal, and no less than 5,000 sequences. You can make a rarefaction curve, to make a more precise decision. But, I like Pat Schloss's advice: If have crappy data (rarefying at anywhere near 5,000 sequences eliminates too much of your data to answer your questions), redo the sequencing or sequence more replicates. If you can't, rarefy at the depth of the lowest samples. Pat says the lowest he has gone is about 500 sequences. This is OK, but you need to be aware that this alters the picture of your data, and acknowledge that in the write up.
...
Here are some figure and statistical samples that can be tweaked to your preferences.
## Stacked Bar Figures
